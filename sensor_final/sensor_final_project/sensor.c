...
PROCESS_THREAD(routing, ev, data) {
	...
	while(1)
	{
		...
		printf("Packet parameters:  dest = 0x%x%x , next_hop(min) = 0x%x%x , message = %s \n", tx_packet.dest.u8[0], tx_packet.dest.u8[1], nextHop.u8[0], nextHop.u8[1], tx_packet.message);
		//broadcast_close(&broadcastConn);
    	...
    	//broadcast_close(&broadcastConn);
		// if(nextHop.u8[1] > ((find_min_arg()+4) & 0xFF))
